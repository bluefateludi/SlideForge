...
- **Token 数来自 trace 聚合**：按每次生成的 trace_id join spans 表取真实 LLM 用量；历史数据或异常查不到 trace 的题按 0 记并标注 `tokens_source`；
- **幻觉核对是子串级**的，不懂语义也不懂换算（材料写「1.86 亿」、页面写「18600 万」会被误记为编造）；
- **重试率是近似值**：任务队列层的重试对 HTTP 不可见，按「生成阶段出现过失败状态的页数 / 总页数」估算；重试类指标波动大，只进报告、不进门禁。

### 回归门禁：`make eval-gate`

观测只回答「现在好不好」，门禁回答「有没有变差」。`make eval-gate` 在评测跑完后把本轮指标与 `backend/eval/baseline.json` 对比，超容差即以退出码 1 结束，可以直接接 CI 或发布前检查：

| 指标 | 容差 |
|---|---|
| 成功率 / Schema 合法率 / 导出成功题数 | 零容差，不允许下降 |
| 评审均分 | 允许下降 ≤ 0.5（采样噪声） |
| Token / 成本 / P95 耗时 | 不允许上涨超过基线 × 1.3 |

两个保护性口径：**题集版本（cases_version）不一致时拒绝对比**——分数跨题集不可比，先重立基线；**基线为 0 的比值型指标自动跳过**（如基线跑在单价未配置期、cost 恒 0），避免把任何非零真值误判为回归。重立基线用 `make eval-baseline`（从库里最近一次同题集的 run 取数，零模型调用）；改题集、配单价、修影响指标的 bug 后都应重立。
